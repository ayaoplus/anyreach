...
| **x** | x.com, twitter.com | Home timeline, profile timeline, list timeline, tweet detail, and longform article detail. Video tweets recover `video.twimg.com` HLS URLs from CDP network events; longform articles are rendered to Markdown from Draft.js rich text. See [adapter-x.md](adapter-x.md) |
