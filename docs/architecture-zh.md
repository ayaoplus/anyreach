...
web-access 证明了 **AI Agent 最好的浏览器工具就是用户自己的浏览器**。但它把太多工作留给了 LLM——每访问一个站点都要从零写 JS、探索 DOM、处理虚拟化渲染。

AnyReach 保留核心洞察（CDP 连接用户日常 Chrome、共享登录态、后台 tab 操作），在此基础上增加：**站点适配器**（可执行的提取代码）、**提示词**（面向 LLM 的站点知识）、**远程注册表**（按需下载）、**批量爬虫**（并发多 URL 提取）。
